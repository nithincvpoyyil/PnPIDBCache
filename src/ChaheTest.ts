...
export function CachingPessimisticRefresh(
  ...
  return (instance: Queryable) => {
    instance.on.init(function (that: Queryable) {
      const newExecute = extend(that, {
          ...
          setTimeout(async () => {
            ...
            try {
              ...
              const emitSend = async (): Promise<any> => {
                ...
                [requestUrl, init] = await that.emit.auth(requestUrl, init);
                ...
                // that.log(`[id:${requestId}] Emitting send`, 0);
                let response = await that.emit.send(requestUrl, init);
                // that.log(`[id:${requestId}] Emitted send`, 0);

                // that.log(`[id:${requestId}] Emitting parse`, 0);
                [requestUrl, response, result] = await that.emit.parse(requestUrl, response, result);
                // that.log(`[id:${requestId}] Emitted parse`, 0);

                // that.log(`[id:${requestId}] Emitting post`, 0);
                [requestUrl, result] = await that.emit.post(requestUrl, result);
              ...
              const emitData = () => {
                ...
                that.emit.data(retVal);
                // that.log(`[id:${requestId}] Emitted data`, 0);
              };

              // that.log(`[id:${requestId}] Beginning request`, 1);

              let [requestUrl, init, result] = await that.emit.pre(that.toRequestUrl(), {}, undefined);
              ...
              // that.log(`[id:${requestId}] Finished request`, 1);
            }
          }, 0);
    ...
    instance.on.pre(function (
      that: Queryable,
      url: string,
      init: RequestInit,
      result: any,
    ): Promise<[string, RequestInit, any]> {
      // Reset refreshCache
      refreshCache = true;
      ...
      if (refreshCache) {
        ...
        that.on.post(async function (url: URL, result: any) {
